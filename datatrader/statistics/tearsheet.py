...
import datatrader.statistics.performance as perf
...
class TearsheetStatistics(AbstractStatistics):
    """
    Visualizza un "one-pager" generato da Matplotlib come spesso
    si trova nei rapporti sulle prestazioni della strategia istituzionali.

    Include una curva equity, una curva di drawdown, una heatmap dei
    rendimenti mensili, un riepilogo dei rendimenti annuali, statistiche
    a livello di strategia e statistiche a livello di trade.

    Include anche un grafico opzionale del Sharpe ratio annualizzato in
    funzione del tempo.
    """
    def __init__(
        ...
        """
        Prevede un gestore di portafoglio.
        """
        self.config = config
    ...
    def update(self, timestamp, portfolio_handler):
        """
        Aggiorna la curva equity e la curva del benchmark che devono
        essere tracciate nel tempo.
        """
    ...
    def get_results(self):
        """
        Restituisce un dizionario con tutti i risultati e statistiche importanti
        """
        # Equity
        ...
        # Sharpe annualizzato a finestra mobile
        ...
        # TODO: bisogna avere il max_drawdown in modo che possa essere stampato alla fine del test
        ...
        # Statistiche del Benchmark se il ticker del benchmark ticker è specificato
        if self.benchmark is not None:
    ...
    def _plot_equity(self, stats, ax=None, **kwargs):
        """
        Visualizza il rendimento cumulativo mobile rispetto ad alcuni benchmark.
        """
   ...
   #     ax.xaxis.set_tick_params(reset=True)
    ...
    def _plot_rolling_sharpe(self, stats, ax=None, **kwargs):
        """
        Visualizza la curva del Sharpe ratio mobile.
        """
        def format_two_dec(x, pos):
            return '%.2f' % x
    ...
    def _plot_drawdown(self, stats, ax=None, **kwargs):
        """
        Visualizza la curva underwater
        """
    ...
    def _plot_monthly_returns(self, stats, ax=None, **kwargs):
        """
        Visualizza la heatmap dei rendimenti mensili.
        """
    ...
    def _plot_yearly_returns(self, stats, ax=None, **kwargs):
        """
        Visualizza la barplot dei rendimenti per anno.
        """
        def format_perc(x, pos):
            return '%.0f%%' % x
    ...
    def _plot_txt_curve(self, stats, ax=None, **kwargs):
        """
        Restituisce le statistiche per la curva di equity.
        """
        def format_perc(x, pos):
            return '%.0f%%' % x
    ...
    def plot_results(self, filename=None):
        ...
        ax_monthly_returns = plt.subplot(gs[3 + offset_index:5, :2])
        ax_yearly_returns = plt.subplot(gs[3 + offset_index:5, 2])
        ax_txt_curve = plt.subplot(gs[5 + offset_index:, 0])
        ax_txt_trade = plt.subplot(gs[5 + offset_index:, 1])
        ax_txt_time = plt.subplot(gs[5 + offset_index:, 2])
